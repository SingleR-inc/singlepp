...
TEST(ScaledRanks, Basic) {
    ...
    singlepp::RankedVector ranks;
    std::vector<double> out (stuff.size());
    singlepp::scaled_ranks(stuff.size(), stuff.data(), ranks, out.data());
...
TEST(ScaledRanks, AllZero) {
    ...
    singlepp::scaled_ranks(all_zeroes.size(), all_zeroes.data(), ranks, out.data());
...
TEST(ScaledRanks, Ties) {
    ...
    singlepp::scaled_ranks(original_size, stuff.data(), ranks, ref.data());
    ...
    // Slapping one onto the end.
    stuff.push_back(stuff[0]);
    std::vector<double> tied(stuff.size());
    singlepp::scaled_ranks(stuff.size(), stuff.data(), ranks, tied.data());
    ...
    singlepp::scaled_ranks(stuff.size(), stuff.data(), ranks, dupped.data());
...
TEST(ScaledRanks, MissingValues) {
    ...
    singlepp::scaled_ranks(stuff.size(), stuff.data(), ranks, ref.data());
    ...
    singlepp::scaled_ranks<true>(missing.size(), missing.data(), ranks, out.data());
...
TEST(ScaledRanks, Subset) {
    ...
    singlepp::scaled_ranks(stuff.data(), sub, ranks, out.data());
    ...
    singlepp::scaled_ranks(sub.size(), stuff2.data(), ranks, out2.data());
    EXPECT_EQ(out, out2);

    // Works with missing values in there.
    stuff[sub[0]] = std::numeric_limits<double>::quiet_NaN();
    singlepp::scaled_ranks<true>(stuff.data(), sub, ranks, out.data());

    stuff2[0] = std::numeric_limits<double>::quiet_NaN();
    singlepp::scaled_ranks<true>(sub.size(), stuff2.data(), ranks, out2.data());
...
TEST(ScaledRanks, CorrelationCheck) {
    ...
    singlepp::scaled_ranks(left.size(), left.data(), ranks, out1.data());

    std::vector<double> out2(right.size());
    singlepp::scaled_ranks(right.size(), right.data(), ranks, out2.data());

    double obs = singlepp::distance_to_correlation(out1.size(), out1.data(), out2.data());
    
    // Manual calculation.
    {
        singlepp::RankedVector ranks1(left.size());
        singlepp::RankedVector ranks2(right.size());
